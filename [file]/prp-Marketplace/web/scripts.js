...
window.addEventListener('message', function(event) {
	...
	switch (event.data.action) {
										...
										<div class="modal-content myinvoices_modal-content">
											<div class="modal-body p-4">
												...
												<div class="d-flex justify-content-center align-items-center" style="margin-top: 10px;">
													...
													<a id="confirmAction" href="#" class="${vehiclesAds[i].plate} btn btn-blue d-block w-50" style="border-radius: 10px; font-size: 30px;" data-bs-dismiss="modal"><i class="fas fa-shopping-cart"></i> BUY</a>
												</div>
											</div>
										</div>
									</div>
		...
		case 'updateVehiclesDropdown':
			vehicles = event.data.vehicles;
			var ownedVehicles = event.data.vehicles;
			var num = ownedVehicles.length;
			var dropdown = `<option value="0" selected>Select a vehicle</option>`;
...
$(document).on('click', "#confirmPlaceAd", function() {
	...
	if (price > 0 && veh > 0) {
		$.post('https://prp-Marketplace/action', JSON.stringify({
			action: "placeAd",
					...
					row += `
						<div class="col-md-3">
							<div class="card item_card d-flex flex-column">
								...
								<a id="buyVehicle" href="#" class="btn btn-blue d-block item_card-btn" data-bs-toggle="modal" data-bs-target="#${vehiclesAds[i].plate}"><i class="fas fa-shopping-cart"></i> BUY</a>
							</div>
						</div>
					`;
